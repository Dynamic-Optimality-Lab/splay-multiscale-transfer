...
- H3T: sizes `[10,12,16,24,32,48,64]` × 10k episodes = 70k; strata `RANDOM_LEGAL, DELETE_BURST_THEN_KEEP, ALTERNATING_KEEP_DELETE, SPINE_VS_BALANCED, OPPOSITE_SPINE, ZIGZIG/ZIGZAG_ENRICHED, BOUNDARY_PAIRING_ENRICHED (target-blind generator), NESTED_INTERVAL_ENRICHED, MIRROR_PAIRED, MOTIF_BLIND_RANDOM_WALK (never seeded from post-freeze counterexamples)`; generator uses only frozen-ontology structural predicates, never candidate residuals (`T59`); bank quarantined, discovery imports fail closed (`STOP-29`).
