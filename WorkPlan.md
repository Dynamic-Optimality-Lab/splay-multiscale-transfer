...
### Gates emitted
`L6_TRANSLATION_FROZEN` (or `L6_TRANSLATION_FAIL`), `CRITICAL_KEEP_CORPUS_CERTIFIED` (+ `KEEP_REGRET_LOCALIZED` / `KEEP_REGRET_DISTRIBUTED` — both non-failures), `KEEP_HEAVY_LEMMA_PROVED` or `…_FALSE_AS_STATED`, `L6_BASELINE_REPRODUCED` (or scoped incomplete).
